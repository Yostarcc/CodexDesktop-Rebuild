 ...
 *
 * 用法:
 *   node tools/fetch-msstore.js <productId> [--ring <ring>] [--market <market>] [--download] [--filter <keyword>]
 *
 * 示例:
 *   node tools/fetch-msstore.js 9plm9xgg6vks
 *   node tools/fetch-msstore.js 9plm9xgg6vks --ring Retail --download --filter x64
 */
...
const { downloadFile, formatSize } = require("./lib/common");
...
async function main() {
  ...
  if (args.length === 0 || args.includes("--help") || args.includes("-h")) {
    console.log(`
用法: node tools/fetch-msstore.js <productId> [选项]
...
示例:
  node tools/fetch-msstore.js 9plm9xgg6vks
  node tools/fetch-msstore.js 9plm9xgg6vks --download --filter x64
`);
    process.exit(0);
  }
  ...
  if (doDownload) {
    ...
    for (const pkg of toDownload) {
      ...
      await downloadFile(pkg.url, dest, { logPrefix: "    下载中:" });
